...
const WAIVER_TEXT = `GOLF CART RENTAL AGREEMENT AND WAIVER OF LIABILITY... (Truncated for brevity, logic preserved)`;

// --- HELPER COMPONENT: CHAIN OF CUSTODY LIST ---
function ChainOfCustodyList({ records }: { records: { time: string; event: string; detail: string }[] }) {
  return (
...
export default function RentalDetail({
  ...
  // Build Chain of Custody Data (Mapped to match Marketing Template style)
      ...
      <div className="max-w-4xl mx-auto pb-12">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-ink-subtle">Secure record of rental #{rental.id.slice(0, 8)}</p>
          </div>
        </div>

        {/* EVIDENCE PACKET CONTAINER (Visual Match: EvidencePacketDrawer) */}
        <div className="bg-surface rounded-dossier-surface border border-rule shadow-dossier-surface overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-paper border-b border-rule">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={handlePrint} icon={<Download className="h-4 w-4" />}>
                Export PDF
              </Button>
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <section>
              ...
              <div className="bg-paper rounded-dossier-control border border-rule p-4 space-y-3">
                <div className="flex justify-between items-baseline">
                  ...
                  {rental.waiver_agreed ? <StampSealed /> : <Badge variant="danger" style="stamp">OPEN</Badge>}
                </div>
                <div className="grid grid-cols-2 gap-4 pt-3 border-t border-rule">
                  <div>
                    ...
                    <p className="text-sm font-medium text-ink">{rental.carts?.name || "Unknown Asset"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono uppercase text-ink-muted">Session Length</p>
                    <p className="text-sm font-medium text-ink">
                      {/* Placeholder calculation or static */}
              ...
              <div className="bg-paper rounded-dossier-control border border-rule p-4 space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-ink">{rental.guest_name || "Unknown Guest"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono uppercase text-ink-muted">IP Address</p>
                    <p className="text-sm font-mono text-ink">{rental.guest_ip || "Not recorded"}</p>
                  </div>
                </div>
            ...
            {(rental.condition_comment || rental.condition_image_url) && (
              <section>
                ...
                <div className="bg-paper rounded-dossier-control border border-accent-warning/30 p-4 relative overflow-hidden">
                  ...
                  {rental.condition_comment && (
                     <div className="mb-4 relative z-10">
                       <p className="text-[10px] font-mono uppercase text-ink-muted mb-1">Guest Statement</p>
                  ...
                  {rental.condition_image_url && (
                    <div className="relative z-10">
                      <p className="text-[10px] font-mono uppercase text-ink-muted mb-2">Attached Evidence</p>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    >
                      ...
                      <div className="p-2 border-t border-rule">
                        ...
                        <div className="flex flex-col gap-0.5 mt-1">
                          <span className="text-[10px] font-mono text-ink-muted break-all">
                  ...
                  <div className="bg-paper rounded-dossier-control border border-rule overflow-hidden">
                    ...
                    >
                       ...
                       <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3">
                          <span className="font-mono text-xs font-semibold text-white">RETURN CONDITION</span>
                       </div>
                    </button>
                    <div className="p-3 border-t border-rule flex justify-between items-center bg-paper">
                       <div>
                       ...
                       <Badge variant="success" style="chip">VERIFIED</Badge>
                    </div>
                  </div>
               ) : (
                  <div className="bg-paper rounded-dossier-control border border-rule border-dashed p-6 text-center">
                     <p className="text-sm text-ink-subtle">No return documentation submitted.</p>
                  </div>
               )}
            </section>

            {/* 6. Waiver Confirmation */}
            <section>
              <h3 className="font-mono text-xs uppercase text-ink-muted tracking-wider mb-3 pl-1">
                Waiver Confirmation
              </h3>
              <div className="bg-paper rounded-dossier-control border border-rule overflow-hidden">
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center ${rental.waiver_agreed ? 'bg-accent-ops/10' : 'bg-accent-legal/10'}`}>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-ink-subtle">
                          {rental.waiver_agreed ? "Guest acknowledged all terms and conditions" : "Liability protection may be limited"}
                        </p>
                      </div>
                    </div>
                    {rental.waiver_agreed && (
                      <Button 
                        ...
                        icon={showWaiver ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                      >
                  ...
                  {rental.waiver_agreed && (
                    <div className="text-xs font-mono text-ink-muted pt-3 mt-3 border-t border-rule">
                      Signed at: {rental.waiver_agreed_at || "Unknown"}
                    </div>
                  )}
                </div>
                ...
                {showWaiver && (
                  <div className="border-t border-rule bg-white p-4 animate-in slide-in-from-top-2">
                    <div className="h-48 overflow-y-auto pr-2 custom-scrollbar">
                      <p className="whitespace-pre-line font-mono text-[10px] leading-relaxed text-ink-subtle">
                        {WAIVER_TEXT}
                      </p>
                    </div>
                  </div>
          ...
          <div className="px-6 py-4 bg-paper border-t border-rule flex items-center justify-between">
             <p className="text-xs text-ink-muted">
               Generated by CartHost Operations Console.
             </p>
